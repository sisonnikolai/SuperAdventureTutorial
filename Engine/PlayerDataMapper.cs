...
namespace Engine
{
    ...
    {
        private static readonly string connectionString = "Data Source=LAPTOP-VMDN6557\\SQLSERVER; Initial Catalog=SuperAdventure; Trusted_Connection=True";
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand savedGameCommand = connection.CreateCommand())
                    {
                        ...
                        //For this program, we would only ever have one row, but this will ensure we only get one record in our
                        //SQL Query results.
                        ...
                        //Create the Player with the saved game values
                    ...
                    using (SqlCommand questCommand = connection.CreateCommand())
                    {
                        ...
                        if(reader.HasRows)
                        {
                            while(reader.Read())
                            {
                                ...
                                //Build the player quest item for this row
        ...
        public static void SaveToDatabase(Player player)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    //Open connection to perform SQL commands
                    connection.Open();
                        ...
                        else
                        {
                            ...
                            using (SqlCommand updateSavedGame = connection.CreateCommand())
                            {
                                ...
                                updateSavedGame.CommandText = "UPDATE SavedGame " +
                                    "SET CurrentHitPoints = @CurrentHitPoints, MaximumHitPoints = @MaximumHitPoints, Gold = @Gold, " +
                                    "ExperiencePoints = @ExperiencePoints, CurrentLocationID = @CurrentLocationID";
                                //Pass the values from the player object to the SQL query using parameters
                                //Using parameters helps make the program more secure and will prevent SQL injection attacks
                    ...
                    //Insert Quests rows from the player object
                    ...
                    foreach (InventoryItem inventoryItem in player.Inventory)
                    {
                        using (SqlCommand insertInventoryCommand = connection.CreateCommand())
                        {
                            ...
                            insertInventoryCommand.Parameters.AddWithValue("@Quantity", SqlDbType.Int);
